...
GO
-----------------------------------------------------------------------------
CREATE FUNCTION FUNCTION_RANDOM_INTEGER()
...
BEGIN
	...
	DECLARE @TEMP AS INT;
		SET @TEMP = (SELECT FLOOR(@rndValue));
	RETURN @rndValue;
END;
GO
-----------------------------------------------------------------------------
CREATE FUNCTION FUNCTION_RANDOM_BETWEEN(@BOTTOM INT, @TOP INT)
RETURNS INT
AS
BEGIN
	RETURN (SELECT CAST(ROUND((@TOP - @BOTTOM) * RandomNumber + @BOTTOM, 0) AS INT) FROM [dbo].RANDOMNUMBER)
END;
GO
-----------------------------------------------------------------------------
/*
